...
namespace Kevsoft.Ssml
{
	public class BreakWriter(ISsml ssml) : FluentSsml(ssml), IBreak, ISsmlWriter
	{
		...
		{
			await writer.WriteStartElementAsync(null, "break", null).ConfigureAwait(false);

			if (_strength != BreakStrength.NotSet)
			{
				var value = BreakStrengthAttributeMap[_strength];

				await writer.WriteAttributeStringAsync(null, "strength", null, value).ConfigureAwait(false);
			}

			if (_duration.HasValue)
			{
				var milliseconds = _duration.Value.TotalMilliseconds;
				await writer.WriteAttributeStringAsync(null, "time", null, $"{milliseconds:F0}ms").ConfigureAwait(false);
			}
