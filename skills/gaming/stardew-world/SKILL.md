...
本 skill 是地点意义与候选解释 owner。它解释 `destination[n]` 的 `label`、`tags`、`reason`、`endBehavior` 和 endpoint candidate 语义：某个地点是什么、为什么值得去、它怎样契合当前意图或人格。
...
- 把 `destination[n]` 读作一个轻量级日程条目：语义化标签、精确坐标、理由、可选朝向，以及可选的结束行为说明。
...
2. 把 `destination[n]` 的 `label`、`tags` 和 `reason` 与你的人格进行比较。
3. 如果地点适合当前意图，用该 `destination[n]` 的精确 `label` 调用 `stardew_move(destination=<label>, reason=<brief intent>)`。
