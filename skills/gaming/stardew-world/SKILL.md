...
- 本 skill 解释 `destination[n]`、`destinationId`、`label`、`tags`、`reason` 和 `schedule_entry[n]` 的地点语义，不替 NPC 决定行动。
- `label` 只用于理解地点意义，不能作为移动命令输入；`destinationId` 只能复制自观察事实。
- 需要更广泛地点知识时由 skill 自己声明：`skill_view(name="stardew-world", file_path="references/stardew-places.md")`。
