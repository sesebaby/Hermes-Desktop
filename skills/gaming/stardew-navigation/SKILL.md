...
- 移动流程是 `stardew_move(destination, reason)`；`destination=<destinationId 精确值>` 必须复制自最新观察。
- mechanical `target(locationName,x,y,source)` 由本地 executor-only 的 `stardew_navigate_to_tile` 执行，父层不要调用或编写该工具参数。
- 需要地图坐标索引时由 skill 自己声明：`skill_view(name="stardew-navigation", file_path="references/index.md")`。
- 长动作进度只用 `stardew_task_status` 轮询。
