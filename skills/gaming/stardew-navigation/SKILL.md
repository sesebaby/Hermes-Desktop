...
- 长动作进度只用无参 `stardew_task_status` 查询当前宿主任务。
...
跨地点移动可以由 bridge 分段执行：当前地图 segment、warp transition、切图后重规划、最终 target segment。发起后必须用无参 `stardew_task_status` 轮询当前宿主任务，重点看 `crossMapPhase`、`currentSegment`、`finalTarget` 和 `lastFailureCode`。
...
- 跟踪状态和失败原因；`commandId`、`traceId` 由宿主记录，不要靠记忆填写。
