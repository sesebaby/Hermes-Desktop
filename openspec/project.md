...
- `openspec/` 下的 proposal、design、tasks、spec、error memory、project 约束等文档，默认优先使用简体中文。
- 如无明确必要，不要先写英文再补中文；应直接以简体中文作为主文本。
- 保留英文的情况只限于代码标识符、命令、路径、协议字段、第三方 API 名称，以及为了避免歧义必须保留的原始术语。
- 如果需要双语，顺序必须是“简体中文在前，英文在后”，且中文内容是主版本，不得把中文写成附注。
- 后续新增或改写 OpenSpec 文档时，评审以“中文是否完整、准确、可执行”为优先标准。
