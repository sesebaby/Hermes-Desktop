...
Stardew host task runner 必须把可恢复的游戏 UI、窗口、动画、菜单和事件等待表示成 task / status 事实。不能仅仅因为游戏正在等待某个可恢复的生命周期条件，就暂停 agent turn、把合法任务打成终态 blocked，或者消耗通用 stale / busy ingress defer 预算。
...
暴露给 agent 的 Stardew host task 动作结果和状态结果，必须包含一个简洁文本 `summary`，说明当前状态，以及剩下的是什么类型的决策需要由 agent 自己做。`status`、`commandId`、`reason`、`errorCode` 以及各类关联标识等机器字段，仍然必须保留，以便测试、UI、日志和后续状态查询使用。
...
对于单个 NPC，host task runner 必须保持只有一个运行中的世界动作槽位。当一个新世界动作和正在进行中的世界动作冲突时，新请求必须返回像 `action_slot_busy` 这样的可观察 blocked 事实，不能偷走当前槽位、替换当前动作，也不能进入一个无界的隐藏队列。
