# Stardew NPC Tool 层分层边界修复 - RALPLAN-DR v3

## Plan Summary

**Plan saved to:** `.omx/plans/stardew-npc-tool-layer-boundary-ralplan-v3.md`
...
2. 以 repo-backed 测试和 bridge 回归清单锁定边界，确保文案收敛不破坏 `path_blocked` / `path_unreachable` / `cross_location_unsupported` / runtime replan 的既有可靠性修复。
...
6. repo-backed 测试不得依赖当前工作目录；真实 `skills/gaming` 定位必须复用或等价实现从 `AppContext.BaseDirectory` 向上寻找 repo root 的模式，等价于 `Desktop/HermesDesktop.Tests/Stardew/StardewPrivateChatWiringTests.cs:103-117` 与 `Mods/StardewHermesBridge.Tests/BridgeMoveCommandQueueRegressionTests.cs:138-150` 的 `ReadRepositoryFile` 思路。

### Decision Drivers

1. 保持最新移动可靠性修复完整，尤其是 stable `path_blocked`、stable `path_unreachable`、stable `cross_location_unsupported`、bounded replan、claim release。
...
**Pros**
- 与 Architect / Critic 反馈完全一致，边界清晰且可验证。
- 能同时保护 description 与 schema 两类护栏。
- 可直接把 `BridgeMoveFailureMapperTests.cs` 与 `BridgeMoveCommandQueueRegressionTests.cs` 纳入冻结清单，保护 stable `path_unreachable`、stable `path_blocked` 与 stable `cross_location_unsupported`。
...
理由：本轮需要的是“owner 写死 + 护栏补齐 + 测试落地”，不是只缩一句 description。Option B 可以把世界语义、移动循环、局部调用契约拆到固定 owner，同时显式保护 schema provenance、`cross_location_unsupported` 与 bridge 错误码回归，范围仍可控。
...
5. `Desktop/HermesDesktop.Tests/Stardew/StardewAutonomyTickDebugServiceTests.cs` 当前大量使用 fixture 写入临时 `skills/gaming/*`，不足以证明真实 repo owner 边界；计划必须新增或调整 repo-backed 测试，并强制这些测试不依赖当前工作目录、必须复用或等价实现从 `AppContext.BaseDirectory` 向上找 repo root 的模式来定位真实 `skills/gaming`。
6. repo-backed 验收不能只靠 direct read 真实 skill 文件；至少一条 repo-backed 测试必须走 `StardewNpcAutonomyPromptSupplementBuilder` 或 service / prompt supplement 注入路径，并使用真实 skill root，证明真实仓库资产会被实际注入到 prompt supplement。
7. 冻结与验证清单必须显式包含 `Mods/StardewHermesBridge.Tests/BridgeMoveFailureMapperTests.cs`，保护 stable `path_unreachable`。
8. 冻结与验证清单必须显式包含 `cross_location_unsupported`，依据 `Mods/StardewHermesBridge.Tests/BridgeMoveCommandQueueRegressionTests.cs:81-93`；该语义要作为独立稳定项，而不是只隐含在“跑 bridge regression”里。
9. 文档步骤必须写成“创建或更新 `docs/星露谷NPC分层边界与HermesCraft对比说明.md`”；执行时若该文件已以 untracked 形式存在，则原位更新，不假设一定新建。
...
1. 先冻结现有可靠性与边界护栏。
   ...
   目标：
   - 列出当前必须保留的稳定项：`moveCandidate/placeCandidate` 来源、非 route-guaranteed、`path_blocked`、`path_unreachable`、`cross_location_unsupported`、claim release、schema provenance。
   - 明确 `BridgeMoveFailureMapperTests.cs` 是 stable `path_unreachable` 的冻结点，`BridgeMoveCommandQueueRegressionTests.cs:81-93` 是 stable `cross_location_unsupported` 与“禁止 silent cross-location teleport”冻结点，不能在边界收敛时被遗漏。
...
4. 把测试从 fixture-only 调整为 fixture + repo-backed 双层验证。
   ...
   - 新增或调整 repo-backed 测试时，真实 `skills/gaming` 定位必须复用或等价实现 `ReadRepositoryFile` / repo root upward-search 模式：从 `AppContext.BaseDirectory` 向上查找，而不是假设当前工作目录正确。
   - 新增或调整至少一组 repo-backed 测试：其中至少一条必须通过 `StardewNpcAutonomyPromptSupplementBuilder` 或 service / prompt supplement 注入路径，指向真实 `skills/gaming` root，验证 world/navigation owner 实际被注入 prompt supplement；direct read 真实 `skills/gaming/stardew-navigation.md`、`skills/gaming/stardew-world/SKILL.md` 可以保留为补充，但不能替代注入路径验证。
...
5. 校准文档与人格层落点，但不扩大范围。
   ...
   - 创建或更新 `docs/星露谷NPC分层边界与HermesCraft对比说明.md`，明确三层 owner、schema 护栏、repo-backed 测试策略、repo root upward-search 约束，以及 `cross_location_unsupported` 冻结点。
...
4. 所有 repo-backed 测试都不依赖当前工作目录；真实 `skills/gaming` 定位必须复用或等价实现从 `AppContext.BaseDirectory` 向上找 repo root 的模式，能力上等价于 `ReadRepositoryFile`。

5. 至少一条 repo-backed 测试必须通过 `StardewNpcAutonomyPromptSupplementBuilder` 或 service / prompt supplement 注入路径并使用真实 skill root，验证 world/navigation owner 被实际注入 prompt supplement；仅 direct read 真实 skill 文件不能单独满足该验收项。

6. `StardewAutonomyTickDebugServiceTests` 继续证明：
   - `placeCandidate` 对模型可见
   - 没有 tool call 时不会 host-side 自动移动
   - 非私聊事件不会直接驱动移动

7. `BridgeMoveFailureMapperTests.cs` 通过，并继续保护初始不可达路径映射为 stable `path_unreachable`，不被收敛为 `path_blocked` 或其他漂移词汇。

8. `BridgeMoveCommandQueueRegressionTests.cs` 继续保护 runtime step blockage 最终走 stable `path_blocked`，并释放 claim / action slot。

9. `BridgeMoveCommandQueueRegressionTests.cs` 中 `cross_location_unsupported` 相关断言继续存在并通过：跨地点移动必须被 block，而不是静默传送或改写为其他失败词。

10. `docs/星露谷NPC分层边界与HermesCraft对比说明.md` 被创建或更新，且内容反映最终 owner 边界、repo-backed 测试策略与 `cross_location_unsupported` 冻结点。
...
2. 风险：继续依赖 fixture 测试，导致真实 `skills/gaming/*` 被改坏却不报警。
   缓解：新增 repo-backed 测试，至少一条走真实 skill root 注入路径，且 repo 定位不依赖当前工作目录。

3. 风险：world/navigation owner 写得太散，执行时再次交叉覆盖。
   缓解：在文档和测试中都用“写死 owner”措辞，不再使用“world 或 navigation”。

4. 风险：边界收敛误伤 bridge 错误码稳定性。
   缓解：显式补入 `BridgeMoveFailureMapperTests.cs` 与 `BridgeMoveCommandQueueRegressionTests.cs` 作为 targeted regression，并把 `cross_location_unsupported` 单独列为冻结项。
...
2. 运行 `dotnet test .\\Desktop\\HermesDesktop.Tests\\HermesDesktop.Tests.csproj -c Debug --filter StardewNpcToolFactoryTests`，确认：
   - description 仍保留最小契约
   - schema 仍保留 `x/y/reason/facingDirection` provenance

3. 运行 `dotnet test .\\Desktop\\HermesDesktop.Tests\\HermesDesktop.Tests.csproj -c Debug --filter StardewAutonomyTickDebugServiceTests`，确认：
   - 至少一条 repo-backed 测试通过 `StardewNpcAutonomyPromptSupplementBuilder` 或 service / prompt supplement 注入路径使用真实 skill root
   - 真实 skill root 的定位不依赖当前工作目录，而是采用 `AppContext.BaseDirectory` upward-search 等价模式
   - `placeCandidate` 可见性、无 tool call 不自动移动、非私聊事件不直接驱动移动均未回退

4. 运行 `dotnet test .\\Mods\\StardewHermesBridge.Tests\\Mods.StardewHermesBridge.Tests.csproj -c Debug --filter BridgeMoveFailureMapperTests`，确认初始不可达仍是 stable `path_unreachable`。

5. 运行 `dotnet test .\\Mods\\StardewHermesBridge.Tests\\Mods.StardewHermesBridge.Tests.csproj -c Debug --filter BridgeMoveCommandQueueRegressionTests`，确认：
   - 运行时阻塞失败仍是 stable `path_blocked`
   - `cross_location_unsupported` 仍是跨地点移动的稳定 block reason
   - 不存在 silent cross-location teleport
   - claim release / action slot release 仍保留

6. 若 prompt supplement / persona / docs 调整带来更广影响，再补跑 `dotnet test .\\Desktop\\HermesDesktop.Tests\\HermesDesktop.Tests.csproj -c Debug`。
...
- 保护最新移动可靠性修复，尤其是 stable `path_blocked` / `path_unreachable` / `cross_location_unsupported`。
...
**Why chosen**
- 该方案以最小必要范围满足 Architect 与 Critic 反馈，并把“谁拥有哪条规则”写死在计划、测试和文档里，后续执行可直接落地。
...
- 以后若新增 repo-backed skill 断言，默认沿用 `AppContext.BaseDirectory` upward-search 模式，而不是依赖当前工作目录。
...
- repo-backed 测试必须先落实 upward-search repo root 模式与至少一条真实注入路径验证，再把 direct read 当补充。
...
3. `StardewAutonomyTickDebugServiceTests` 中至少一条 repo-backed 测试走真实 skill root 注入路径，且 skill root 解析不依赖当前工作目录。
4. `BridgeMoveFailureMapperTests` 仍保护 stable `path_unreachable`。
5. `BridgeMoveCommandQueueRegressionTests` 仍保护 stable `path_blocked`、stable `cross_location_unsupported` 和 claim release。
...
**Launch hints:**
- Ralph：`$ralph 按 .omx/plans/stardew-npc-tool-layer-boundary-ralplan-v3.md 执行，先锁定 schema provenance、cross_location_unsupported 与 repo-backed 注入路径测试，再收敛文案`
- Team：`$team 按 .omx/plans/stardew-npc-tool-layer-boundary-ralplan-v3.md 执行，3 lane 并行：tool、skills/docs、tests`

## Applied Feedback Changelog
...
1. 在 Requirements、Implementation Steps、Acceptance Criteria、Verification Steps 明确加入硬约束：repo-backed 测试不得依赖当前工作目录，真实 `skills/gaming` 定位必须复用或等价实现 `AppContext.BaseDirectory` upward-search 的 repo root 解析模式。
2. 把 `cross_location_unsupported` 显式加入冻结清单、验收项、验证项，并单独指出其禁止 silent cross-location teleport 的语义来源于 `BridgeMoveCommandQueueRegressionTests.cs:81-93`。
3. 收紧 repo-backed 测试验收：至少一条 repo-backed 测试必须走 `StardewNpcAutonomyPromptSupplementBuilder` 或 service / prompt supplement 注入路径并使用真实 skill root；direct read 真实 skill 文件只能作为补充，不能替代注入路径验证。
4. 将 Verification Steps 中的测试执行说明收紧为完整 `dotnet test` 命令，避免执行者自行猜测过滤范围。
